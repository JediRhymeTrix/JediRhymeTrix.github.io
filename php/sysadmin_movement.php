...
if(isset($_POST['status']) || isset($_POST['status1'])) {
    ...
    $query = mysqli_query($con,"select * from movement status like '%".$status."%' or status like '%".$status1."%'");
...
if (isset($_POST['active_id'])) {
    ...
    $query = mysqli_query($con,"update movement set status='closed' where req_id=$id");
